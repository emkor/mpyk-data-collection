...
for f in $(find ${CSV_DIR} -name "*.csv" | grep -v $(date --iso-8601 --utc)); do echo "Archiving file $f ..." && zip --junk-paths "$f.zip" ${f} && b2 upload-file --noProgress ${B2_BUCKET} "$f.zip" $(basename "$f.zip") && rm ${f} && rm "$f.zip"; done
